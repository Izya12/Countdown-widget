...
Документ сохраняет исходную спецификацию от 2026-09-15. «Поддержать», «делаем» и критерии ниже описывают целевое поведение, а не результаты испытаний. Актуальная реализация и проверки — [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md). Текущий toolchain: AGP 9.3.3 / Gradle 9.5.0 / JetBrains JDK 21; таблицы ниже с AGP 8.13.2 описывают первоначальный выбор. Проверенные положения Android отделены ссылками от проектных решений. Реестр источников и непроверенных интеграционных деталей — [SOURCES.md](SOURCES.md).
